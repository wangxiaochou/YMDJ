...
public class LunarUtils {
    ...
    /**
     * @param lunar 农历日期
     * @return String of 天:初一、初二...
     */
    public static String lunarDayToString(Lunar lunar) {
        // 农历部分假日
        String[] lunarHoliday = new String[]{"0101 春节", "0115 元宵节",
                "0505 端午节", "0707 七夕", "0715 中元节", "0815 中秋节", "0909 重阳节",
                "1208 腊八节", "1224 小年", "1230 除夕"};

        for (String lunarHolidayDetail : lunarHoliday) {
            String[] info = lunarHolidayDetail.split(" ");
            String format = String.format("%02d%02d".toLowerCase(), lunar.lunarMonth, lunar.lunarDay);
            if (format.trim().equals(info[0].trim())) {
                return info[1];
            }
        }

        // 公历部分节假日
        String[] solarHoliday = new String[]{"0101 元旦", "0214 情人节",
                "0308 妇女节", "0312 植树节", "0401 愚人节", "0501 劳动节", "0504 青年节",
                "0512 护士节", "0601 儿童节", "0701 建党节", "0801 建军节", "0910 教师节",
                "1001 国庆节", "1031 万圣节", "1111 光棍节", "1224 平安夜", "1225 圣诞节"};

        Solar solar = LunarToSolar(lunar);
        for (String solarHolidayDetail : solarHoliday) {
            String[] info = solarHolidayDetail.split(" ");
            String format = String.format("%02d%02d".toLowerCase(), solar.solarMonth, solar.solarDay);
            if (format.trim().equals(info[0].trim())) {
                return info[1];
            }
        }
        ...
        if (lunarDay == 1) {
            ...
            final String[] month = {"一", "二", "三", "四", "五", "六", "七", "八", "九", "十", "冬", "腊"};
            if (lunar.lunarMonth == 1) {
                stringBuilder.append("春节");
            } else {
                stringBuilder.append(month[lunar.lunarMonth - 1]).append("月");
            }
            return stringBuilder.toString();
        }
        String[] DayTen = new String[]{"初", "十", "廿", "初", "二", "三"};
        if (lunarDay % 10 == 0) {
            stringBuilder.append(DayTen[(lunarDay / 10) + 2]);
        } else {
            stringBuilder.append(DayTen[lunarDay / 10]);
        }
        final String[] day = {"十", "一", "二", "三", "四", "五", "六", "七", "八", "九"};
        stringBuilder.append(day[lunarDay % 10]);
        return stringBuilder.toString();
    }
