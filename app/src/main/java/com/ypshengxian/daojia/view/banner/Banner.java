...
import com.ypshengxian.daojia.loader.ILoader;
import com.ypshengxian.daojia.loader.IOnImageLoadListener;
...
public class Banner extends FrameLayout implements OnPageChangeListener {
    public String tag = "opsrc_banner";
    // modified by mos, 2017.07.05，若仅显示圆点，可设置圆点布局的marginBottom。
    private int mIndicatorMarginBottomIfCircle;
    ...
    private int mIndicatorSelectedResId = R.drawable.opsrc_banner_gray_radius;
    private int mIndicatorUnselectedResId = R.drawable.opsrc_banner_white_radius;
    private int mLayoutResId = R.layout.opsrc_banner;
    ...
    private IOnImageLoadListener mOnBitMapLoadListener;
...
//                Log.i(tag, "curr:" + currentItem + " count:" + count);
    ...
    private void handleTypedArray(Context context, AttributeSet attrs) {
        ...
        mIndicatorSelectedResId = typedArray.getResourceId(R.styleable.Banner_indicator_drawable_selected, R.drawable.opsrc_banner_gray_radius);
        mIndicatorUnselectedResId = typedArray.getResourceId(R.styleable.Banner_indicator_drawable_unselected, R.drawable.opsrc_banner_white_radius);
    ...
    public Banner setImageLoader(ILoader imageLoader) {
    ...
    public Banner setIndicatorGravity(int type) {
        switch (type) {
            ...
            case BannerConfig.RIGHT:
                ...
                break;
            case BannerConfig.TOP:
                this.gravity=Gravity.TOP | Gravity.RIGHT;
            default:
                break;
        }
        return this;
    ...
    private void setImageList(List<?> imagesUrl) {
        ...
        for (int i = 0; i <= count + 1; i++) {
            ...
            if (imageLoader != null) {
                if (null != mOnBitMapLoadListener) {
                    final ImageView finalImageView = imageView;
                    imageLoader.loadListener(context, url.toString(), new IOnImageLoadListener() {
                        @Override
                        public void onSuccess(Object model, Bitmap bitmap) {
                            finalImageView.setImageBitmap(bitmap);
                            mOnBitMapLoadListener.onSuccess(model, bitmap);
                        }

                        @Override
                        public void onFail(Throwable throwable) {
                            mOnBitMapLoadListener.onFail(throwable);
                        }
                    });
                } else {
                    imageLoader.loadNet(imageView, url.toString());
                }
            } else
    ...
    private void createIndicator() {
        ...
        // modified by mos, 2017.07.05，若仅显示圆点，可设置圆点布局的marginBottom。
        if (bannerStyle == BannerConfig.CIRCLE_INDICATOR && mIndicatorMarginBottomIfCircle != 0) {
            RelativeLayout.LayoutParams layoutParams = (RelativeLayout.LayoutParams) indicator.getLayoutParams();
            layoutParams.setMargins(0, 0, 0, mIndicatorMarginBottomIfCircle);
...
//        Log.i(tag, ev.getAction() + "--" + isAutoPlay);
     ...
     * 设置图片加载的监听，每次滑动会回调3次，
     *
     * @param bitMapLoadListener 回调
     */
    public void setOnBitMapLoadListener(IOnImageLoadListener bitMapLoadListener) {
        mOnBitMapLoadListener = bitMapLoadListener;
    }
    ...
    /**
     * 设置圆点模式下的marginBottom
     *
     * @param marginBottom 底部间距
     */
    public void setIndicatorMarginBottomIfCircle(int marginBottom) {
        mIndicatorMarginBottomIfCircle = marginBottom;
    }
